...
SF_DETECTOR_IMAGE=${REGISTRY}sf-detector:${SF_VERSION}
SF_EXTRACTOR_IMAGE=${REGISTRY}sf-extractor:${SF_VERSION}

# stop all services before migration
docker compose down --remove-orphans

# Set the number of face detector/extractor containers to run (default is 3)
...
# RUN DRY-RUN OF SET STATE ERROR FOR MIGRATED FACES
docker run --rm --name sf_admin \
    --volume "$(pwd)/iengine.lic:/etc/innovatrics/iengine.lic" \
    --network sf-network \
    "${SF_ADMIN_IMAGE}" \
