...
docker exec -it rmq3 /opt/rabbitmq/sbin/rabbitmqctl add_user mqtt mqtt || true
docker exec -it rmq3 /opt/rabbitmq/sbin/rabbitmqctl set_user_tags mqtt administrator || true
docker exec -it rmq3 /opt/rabbitmq/sbin/rabbitmqctl set_permissions -p "/" mqtt ".*" ".*" ".*" || true

docker exec -it rmq3 /opt/rabbitmq/sbin/rabbitmqctl stop_app
docker exec -it rmq3 /opt/rabbitmq/sbin/rabbitmqctl reset
docker exec -it rmq3 /opt/rabbitmq/sbin/rabbitmqctl join_cluster rabbit@rmq1
docker exec -it rmq3 /opt/rabbitmq/sbin/rabbitmqctl start_app
