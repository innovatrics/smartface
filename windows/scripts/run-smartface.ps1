...
function Initialization {
	...
	Write-Host "SmartFace Evaluation DEMO for Microsoft Windows"
	Write-Output ""
	Write-Output "Initialization. Analyzing hardware..."
	
	# Find out the number of cores
	$global:cpuCores = (Get-CimInstance -ClassName Win32_Processor).NumberOfCores
	Write-Output "Number of CPU Cores: $cpuCores"

	# Find out the amount of RAM
	$global:totalMemory = (Get-CimInstance -ClassName Win32_ComputerSystem).TotalPhysicalMemory
...
function RunUI
{
	...
	$lbl_license_header2.Location = (New-Object -TypeName System.Drawing.Point -ArgumentList @([System.Int32]12,[System.Int32]218))
	...
	# Ensure the form is shown and focused
	$Form1.Add_Shown({
		$Form1.Activate()    # Brings the form into focus
		$Form1.TopMost = $true  # Ensures the form stays on top
	})
...
if (-not (Test-IsAdmin)) {
    Write-Error "Administrator privileges are missing..."
    Exit
}
